...
if __name__ == "__main__":
    df = pd.read_csv(r'E:\untitled\create2\data\CN0CN2CN3_TR.csv').values
...
class TrAdaboost:
    ...
    def fit(self, x_source, x_target, y_source, y_target):
        ...
        #result to save the prediction
        result = np.ones([row_source + row_target, self.N])
        for i in range(self.N):
            weights = self._calculate_weight(weights) #normalized the weights each time
